...
import { ArrowLeft, Navigation, Clock, Activity, Map as MapIcon } from "lucide-react";
...
export default function History() {
  ...
  if (loadingDevice) return <div className="p-8 text-center">Loading device...</div>;
  if (!device) return <div className="p-8 text-center text-rose-500">Device not found</div>;
        ...
        <div className="flex justify-between items-end">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <Badge variant="outline" className="uppercase">{device.type}</Badge>
              <Badge className={
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 min-h-0">
        <div className="md:col-span-2 flex flex-col border border-border/50 rounded-lg overflow-hidden bg-card/50">
          ...
          <div className="flex-1 overflow-auto p-6">
            {loadingHistory ? (
              <p className="text-center text-muted-foreground font-mono">Loading history...</p>
            ) : history.length === 0 ? (
              <p className="text-center text-muted-foreground font-mono">No position history recorded.</p>
            ) : (
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-mono font-bold text-sm text-foreground">
                            {format(new Date(pos.timestamp), 'MMM dd, HH:mm:ss')}
                          </span>
                          {pos.speed > 0 ? (
                            ...
                            <Badge variant="outline" className="text-muted-foreground text-[10px] font-mono">Stopped</Badge>
                          )}
                        </div>
        ...
        <div className="space-y-6 overflow-y-auto">
          <Card className="bg-card border-border/50 shadow-none">
            <CardContent className="p-5">
              <h3 className="font-mono font-semibold mb-4 text-sm uppercase tracking-wider text-muted-foreground">Device Details</h3>
              <dl className="space-y-3 text-sm">
                <div className="flex justify-between border-b border-border/50 pb-2">
                  <dt className="text-muted-foreground">Group</dt>
                  <dd className="font-medium font-mono">{device.groupName || 'Unassigned'}</dd>
                </div>
                <div className="flex justify-between border-b border-border/50 pb-2">
                  <dt className="text-muted-foreground">Speed Limit</dt>
                  <dd className="font-medium font-mono">{device.speedLimit ? `${device.speedLimit} km/h` : 'None'}</dd>
                </div>
                <div className="flex justify-between pb-2">
                  <dt className="text-muted-foreground">Registered</dt>
                  <dd className="font-medium font-mono text-xs">{format(new Date(device.createdAt), 'MMM dd, yyyy')}</dd>
                </div>
              </dl>
            </CardContent>
          </Card>
